...
Registrations::Registrations(Type type_)
  :m_type(type_)
{
    assert( type_ == TOPIC_SUBSCRIPTIONS || type_ == TOPIC_PUBLICATIONS || type_ == SERVICE || type_ == PARAM_SUBSCRIPTIONS);
}
...
std::vector<std::vector<std::string>> Registrations::getState() const
{
    ...
    for (const auto& pair: map)
    {
        ...
        //value.AddRange(pair.second[0]);
        #ifdef IMPLEMENT
        value.insert(value.end(), pair.second[0].begin(), pair.second[0].end());
        #endif
  ...
  const std::string& caller_api, const std::string& service_api)
{
    ...
    if (providerIt != map.end() && service_api.empty())
    {
        ...
        {
            providers.push_back(tmplist); //CHANGEING ID || API FOR EXISTING TOPIC? IF SOMETHING BREAKS, THIS IS IT
        }
        #endif
    }
    else if (service_api.empty())
    {
         //map[key] = providers = new std::vector<std::vector<std::string>>();
...
ReturnStruct Registrations::unregisterObj(std::string key, std::string caller_id, std::string caller_api, std::string service_api)
{
    std::stringstream ss;
    if (!service_api.empty())
    {
...
void Registrations::unregister_all(const std::string& caller_id)
{
    ...
    {
        std::remove_if(providers.begin(), providers.end(), [&](const Record& r) {
