...
*/
    ...
    {
        std::string par = *par_iter;
        format_paragraph(os, par, first_column_width, line_length);
...
unsigned options_description::get_option_column_width() const
{
    ...
    for (i = 0; i < m_options.size(); ++i)
    {
        ...
        std::stringstream ss;
        ss << "  " << opt.format_name() << ' ' << opt.format_parameter();
        width = std::max<unsigned>(width, static_cast<unsigned>(ss.str().size()));
    }

    /* Get width of groups as well*/
    for (unsigned j = 0; j < groups.size(); ++j)
        width = std::max<unsigned>(width, groups[j]->get_option_column_width());

    /* this is the column were description should start, if first
        column is longer, we go to a new line */
    const unsigned start_of_description_column = m_line_length - m_min_description_length;

    width = std::min<unsigned>(width, start_of_description_column-1);
